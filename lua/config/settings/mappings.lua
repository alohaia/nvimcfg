...
global_mappings = {
    ...
    -- gt gT
    {'n', '<C-t><C-n>', '<Cmd>tabnew<CR>'},
    { {'n', 'x'}, '_', '<Cmd>tabprevious<CR>' },
    { {'n', 'x'}, '+', '<Cmd>tabnext<CR>' },
    {'n', '<C-t><C-c>', '<Cmd>tabclose<CR>'},
    {'n', '<C-t><C-o>', '<Cmd>tabonly<CR>'},
    {'n', ']t', '<Cmd>tablast<CR>'},
    {'n', '[t', '<Cmd>tabfirst<CR>'},
    ...
    {'n', '<leader>e', ':e $HOME/.config/nvim/lua/config/<C-z>', {silent = false}},
...
-- set global mappings ---
--------------------------
if type(global_mappings) == 'table' then
    for _,map in pairs(global_mappings) do
        vim.keymap.set(
            map[1], map[2], map[3],
            vim.tbl_deep_extend('force', default_map_opts, map[4] or {})
        )
    end
end
